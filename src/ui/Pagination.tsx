...
<div className="flex mx-4 gap-2">
      ...
      {getPageList().map((p) =>(p!="ellipsis"? <button className={clsx("bg-transparent  font-semibold min-w-[40px] border border-gray-300 rounded-md px-2 py-1  focus:outline-none",currentPage==p&&"text-primary border-primary")}
        onClick={()=>onPageChange(p)}
        
      >
                {p}
            </button>:<span key={p} className="px-2">
            ...
          </span>)
      )}
      </div>
      ...
      >
          ...
          <input  type="number" onBlur={(e)=>onPageChange(Number(e.target.value))} 
